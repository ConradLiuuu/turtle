...
class Ctrler:
    ...
    def cal_vel_by_turtle(self):
        self.__v = self.__k_lo * self.euclidean_distance()
        self.__w = 10 * (self.steering_angle() - self.fi + self.__PI)
        #self.__w = 10 * (self.__alpha)

    def show_status(self):
        print("position = (%f, %f, %f)" % (self.x, self.y, self.fi*180/self.__PI))
        print("linear vel = %f, angular_vel = %f" % (self.__v, self.__w))
        print("----------------------------------")
    ...
    def rotate_itself(self):
        if self.fi >= 2*self.__PI:
            self.fi = self.fi - 2*self.__PI
        elif self.fi <= -(2*self.__PI):
            self.fi = self.fi + 2*self.__PI

        if int(round(self.fi)) != int(round(self.dst_fi)):
            self.__v = 0
        else:
            self.__w = 0
